...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h2 class="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface">Catat Pengeluaran Baru</h2>
        <p class="font-body-md text-body-md text-on-surface-variant mt-1">Isi formulir di bawah untuk mencatat pengeluaran operasional.</p>
    </div>
        ...
        <form action="{{ route('pengeluaran.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3" 
                          class="w-full px-4 py-2 bg-surface rounded-lg border border-outline-variant/50 focus:border-primary focus:ring-1 focus:ring-primary outline-none font-body-md text-body-md text-on-surface">{{ old('keterangan') }}</textarea>
            </div>
            ...
            <div>
                ...
                <p class="text-xs text-on-surface-variant mt-1">Maksimal 2MB. Format: JPG, PNG, WEBP.</p>
                @error('foto_nota')
                    <p class="text-error text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="pt-4 border-t border-outline-variant/30 flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-lg bg-primary text-on-primary font-label-md text-label-md hover:bg-primary/90 shadow-sm transition-all">Simpan Pengeluaran</button>
            </div>
        </form>
    </div>
</div>
